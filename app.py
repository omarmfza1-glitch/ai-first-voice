...
# ============================================================================
# 7. نقاط النهاية الرئيسية (تم إعادة الهيكلة)
# ============================================================================
@app.post("/twilio/voice")
async def voice_handler(request: Request, x_twilio_signature: Optional[str] = Header(None)):
    """
    نقطة الدخول الأولى للمكالمة. مسؤولة فقط عن التحية وإعادة التوجيه.
    """
    if twilio_validator:
    ...
    if call_sid not in CALL_STATE: CALL_STATE[call_sid] = {"turn": 0, "from_number": from_number}

    twiml = f"""<?xml version="1.0" encoding="UTF-8"?>
<Response>
    <Say language="ar-SA" voice="Polly.Zeina">مرحبًا بكم في مركز الاتصال الذكي.</Say>
    <Redirect method="POST">{BASE_URL}/twilio/stream</Redirect>
</Response>""".strip()
...
async def stream_handler(request: Request, x_twilio_signature: Optional[str] = Header(None)):
    """
    هذه النقطة مسؤولة فقط عن بدء بث الصوت.
    """
    if twilio_validator:
        form_params = await request.form(); url = str(request.url)
        if "x-forwarded-proto" in request.headers: url = url.replace("http://", f"{request.headers['x-forwarded-proto']}://")
    ...
    logger.info(f"🎧 Stream handler: Starting media stream for call {call_sid}")
...
async def media_stream_handler(ws: WebSocket):
        ...
        try:
            while True:
                ...
                if event == "start":
                    ...
                    logger.info(f"⏹️ WS Receiver: Stream stopped. Ending queue.")
    ...
    async def stt_sender(queue: asyncio.Queue):
        yield speech_types.StreamingRecognitionConfig(
            config=speech_types.RecognitionConfig(encoding=speech_types.RecognitionConfig.AudioEncoding.LINEAR16, sample_rate_hertz=8000, language_code="ar-SA", model="telephony", use_enhanced=True),
            interim_results=False)
        while True:
            chunk = await queue.get()
            if chunk is None: break
            yield speech_types.StreamingRecognizeRequest(audio_content=chunk)
        logger.info("STT sender finished.")
...
async def _handle_user_turn(call_sid: str, user_text: str):
    ...
    if twilio_client and not call_sid.startswith("local_"):
        try:
            # تحديث: إعادة التوجيه إلى /stream لبدء الاستماع مجددًا
            twiml_response = f"""<?xml version="1.0" encoding="UTF-8"?><Response>{f'<Play>{mp3_url}</Play>' if mp3_url else f'<Say language="ar-SA" voice="Polly.Zeina">{final_reply_text}</Say>'}<Redirect method="POST">{BASE_URL}/twilio/stream</Redirect></Response>"""
