# app.py — مركز اتصال ذكي متكامل (النسخة النهائية v8 - تجربة وتشخيص)
# -*- coding: utf-8 -*-
...
try:
    ...
    logger.info("✅ Google Cloud Speech SYNC Client initialized successfully.")
except (ImportError, Exception) as e:
    logger.warning(f"⚠️ Google Cloud Speech Client could not be initialized: {e}")
...
def stt_processor(audio_generator: Callable[[], Optional[bytes]], call_sid: str):
    logger.info(f"STT PROCESSOR [{call_sid}]: Thread started.")
    try:
        if not speech_client or not speech_types:
            logger.error(f"STT PROCESSOR [{call_sid}]: Speech client not available.")
            return

        requests = (speech_types.StreamingRecognizeRequest(audio_content=chunk) for chunk in audio_generator())
        ...
        logger.info(f"STT PROCESSOR [{call_sid}]: Awaiting responses...")
        
        for response in responses:
            if not response.results or not response.results[0].alternatives: continue
            transcript = response.results[0].alternatives[0].transcript.strip()
            if transcript:
                logger.info(f"🎤 STT Final in thread [{call_sid}]: '{transcript}'")
                asyncio.run(_handle_user_turn(call_sid, transcript))
    except Exception as e:
        logger.error(f"STT PROCESSOR [{call_sid}]: Error: {e}")
    finally:
        logger.info(f"STT PROCESSOR [{call_sid}]: Thread finished.")
...
async def media_stream_handler(ws: WebSocket):
    ...
    async def ws_receiver():
        ...
        try:
            while True:
                ...
                logger.info(f"WS RECEIVER: Received event '{event}'")
                if event == "start":
                    ...
                    logger.info(f"⏹️ WS RECEIVER: Stream stopped for {call_sid}."); break
            await audio_queue.put(None)
        except WebSocketDisconnect: logger.info("WS receiver disconnected.")
        finally: await audio_queue.put(None)
    ...
    # انتظر حتى يتم الحصول على call_sid
    while call_sid is None: await asyncio.sleep(0.1)

    loop = asyncio.get_running_loop()
    stt_task = loop.run_in_executor(None, stt_processor, audio_generator, call_sid)
