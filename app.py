...
async def voice_handler(request: Request, x_twilio_signature: Optional[str] = Header(None)):
    ...
    if twilio_validator:
        ...
        if not twilio_validator.validate(url, form_params, x_twilio_signature or ""):
            logger.warning(f"❌ Twilio signature validation failed for /twilio/voice"); raise HTTPException(status_code=403, detail="Invalid Twilio Signature")
    ...
    twiml = f"""<?xml version="1.0" encoding="UTF-8"?><Response><Start><Stream url="{ws_url}"><Parameter name="callSid" value="{call_sid}"/></Stream></Start>{'<Say language="ar-SA" voice="Polly.Zeina">مرحبًا بكم في مركز الاتصال الذكي. كيف يمكنني مساعدتك اليوم؟</Say>' if first_turn else ''}<Pause length="60"/></Response>""".strip()
    ...
    try:
        for resp in stt_responses:
            if resp.results and resp.results[0].alternatives:
                ...
                if transcript: logger.info(f"🎤 STT Final [{call_sid}]: '{transcript}'"); await _handle_user_turn(call_sid, transcript)
    except Exception as e: logger.error(f"STT consumption error for {call_sid}: {e}")
async def _handle_user_turn(call_sid: str, user_text: str):
    ...
    if not call_state: logger.warning(f"No state found for call {call_sid}. Aborting turn."); return
    intent, tool_called, tool_result, reply_text = await _llm_plan_and_reply(user_text, call_state)
    final_reply_text = reply_text or "عذرًا، لم أفهم. هل يمكنك إعادة الصياغة؟"; mp3_url = await _synthesize_tts(final_reply_text)
    if twilio_client and call_sid and not call_sid.startswith("local_"):
        try:
            ...
            twilio_client.calls(call_sid).update(twiml=twiml_response); logger.info(f"✅ Call updated for {call_sid} with new TwiML.")
...
async def _llm_plan_and_reply(user_text: str, call_state: dict) -> tuple:
    ...
    from_number = call_state.get("from_number", ""); SYSTEM_PROMPT = f"أنت مساعد صوتي ذكي في مركز اتصال لشركة اتصالات سعودية. مهمتك هي فهم نية العميل بدقة واستخدام الأدوات المتاحة للإجابة على استفساراته. كن مهذبًا، موجزًا، وتحدث بالعربية الفصحى المبسطة. رقم هاتف المتصل هو: {from_number}"
    ...
    except Exception as e: logger.error(f"LLM Error: {e}"); return "error", None, None, "عذرًا، أواجه صعوبة فنية. يرجى المحاولة مرة أخرى."
...
if __name__ == "__main__":
    import uvicorn
    logger.info("="*50); logger.info(f"🚀 Starting Smart Call Center in development mode on http://0.0.0.0:{PORT}"); logger.info(f"🧪 Test Mode: {'ON' if TEST_MODE else 'OFF'}"); logger.info("="*50)
